...
def get_peak_memory():
    if torch.cuda.is_available():
        return torch.cuda.max_memory_allocated() / 1024 / 1024  # Returns MB
    return 0.0
...
def run_sadam_experiment(name, device, train_loader, test_loader, epochs, k, sigma, lgi_lambda):
    ...
    return losses, opt.lgi_history, total_time
...
if __name__ == "__main__":
    sys.stdout = Logger('training_log_prox_adamw_sadam_CIFAR100_ablation.txt')
    ...
    # Plot Baselines
    ax1.plot(l_prox, color='orange', alpha=0.6, label='Prox-SGD (Baseline)')
    ax1.plot(l_adamw, color='purple', alpha=0.6, label='AdamW (Baseline)')
    ...
    # Plot LGI for Ours (optional, or maybe for all?)
    # Just Ours for clarity
    ax2 = ax1.twinx()
    ax2.set_ylabel('LGI Score (Ours)', color='green')
    if lgi_sadam:
        ax2.fill_between(range(len(lgi_sadam)), lgi_sadam, color='green', alpha=0.1)
    
    plt.title("S-Adam vs Baselines vs Ablations")
    plt.tight_layout()
    plt.savefig('sadam_vs_proxsgd_adamw_CIFAR100_ablation.png', dpi=300)
    print("Saved plot to sadam_vs_proxsgd_adamw_CIFAR100_ablation.png")
    plt.show()
