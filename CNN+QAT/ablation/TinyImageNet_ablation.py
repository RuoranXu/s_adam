...
def run_sadam_experiment(name, device, train_loader, test_loader, epochs, k, sigma, lgi_lambda):
    ...
    return losses, opt.lgi_history, total_time, test_acc, memory_usage
...
def run_comparison():
    ...
    print("=" * 100)
    print(f"{'Method':<30} | {'Total Time (s)':<15} | {'Final Acc (%)':<15} | {'Max Memory (MB)':<15}")
    print("-" * 100)
    print(f"{'Prox-SGD (Base)':<30} | {total_time_prox:.4f}          | {acc_prox:.2f}          | {memory_prox:.2f}")
    print(f"{'AdamW (Base)':<30} | {total_time_adamw:.4f}          | {acc_adamw:.2f}          | {memory_adamw:.2f}")
    print(f"{'S-Adam (Ours)':<30} | {t_sadam:.4f}          | {acc_sadam:.2f}          | {m_sadam:.2f}")
    print(f"{'Ablation A (No Brake)':<30} | {t_a:.4f}          | {acc_a:.2f}          | {m_a:.2f}")
    print(f"{'Ablation B (Low Res)':<30} | {t_b:.4f}          | {acc_b:.2f}          | {m_b:.2f}")
    print(f"{'Ablation C (Near Point)':<30} | {t_c:.4f}          | {acc_c:.2f}          | {m_c:.2f}")
    print("-" * 100)
    print("=" * 100 + "\n")
...
if __name__ == "__main__":
    sys.stdout = Logger('training_log_prox_adamw_sadam_tinyimagenet_ablation.txt')
    ...
    plt.tight_layout()
    plt.savefig('sadam_vs_proxsgd_adamw_tinyimagenet_ablation.png', dpi=300)
    print("Saved plot to sadam_vs_proxsgd_adamw_tinyimagenet_ablation.png")
    plt.show()
