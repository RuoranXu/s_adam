...
def run_comparison():
    ...
    if torch.cuda.device_count() > 1:
        model_prox = nn.DataParallel(model_prox)
    ...
    if torch.cuda.device_count() > 1:
        model_adamw = nn.DataParallel(model_adamw)
    ...
    if torch.cuda.device_count() > 1:
        model_sadam = nn.DataParallel(model_sadam)
...
if __name__ == "__main__":
    sys.stdout = Logger('training_log_CIFAR100_resnet_learning_batch64.txt')
    ...
    plt.tight_layout()
    plt.savefig('sadam_vs_proxsgd_adamw_CIFAR100_resnet_batch64.png', dpi=300)
    print("Saved plot to sadam_vs_proxsgd_adamw_CIFAR100_resnet_batch64.png")
    plt.show()
